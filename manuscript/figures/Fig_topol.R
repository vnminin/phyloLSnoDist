...
n.sites4 <- n.sites[c(1,2,3,4)] # 4/9/20 change indices if re-run
...
pdf(here("analysis", "Fig_topol.pdf"),height=10,width=8)
par(mfrow=c(3,1),oma=c(1,2,0,0),mar=c(3,2,5,2))
...
# legend("topleft",pch=c(15,17,16),c(expression(L[1]),expression(L[2]),"MLE"), cex=1.5)
legend("topleft",pch=c(15,17,16),c("OLS", "New LS","MLE"), cex=2)
#text(1.25, .925, "L", vfont=c("script","bold"), cex=2)
#text(1.25, .81, "L", vfont=c("script","bold"), cex=2)
#text(1.39, .89, "1", cex=1, font=2)
#text(1.39, .775, "2", cex=1, font=2)
box()
...
rect(3.5,-1,6.5,1.05,border="gray90",col="gray90")
rect(9.5,-1,12.5,1.05,border="gray90",col="gray90")
bxp(res5.box,outline=F,medpch=rep(c(15,17,16),4),medcex=1.7,medlty="blank",whisklty=1,whisklwd=1.5,staplewex=1,staplelwd=1.5,boxwex=0.12,axes=F,add=T)
axis(2,at=seq(0,1,by=0.25),label=c(0,25,50,75,100), cex.axis=1.5)
...
rect(3.5,-1,6.5,1.05,border="gray90",col="gray90")
rect(9.5,-1,12.5,1.05,border="gray90",col="gray90")
bxp(res6.box,outline=F,medpch=rep(c(15,17,16),4),medcex=1.7,medlty="blank",whisklty=1,whisklwd=1.5,staplewex=1,staplelwd=1.5,boxwex=0.12,axes=F,add=T)
axis(2,at=seq(0,1,by=0.25),label=c(0,25,50,75,100), cex.axis=1.5)
...
#######################3
# 4 taxa results only
pdf(here("analysis", "Fig_topol4.pdf"),height=6,width=12)
...
axis(1,at=seq(2,11,by=3),label=n.sites)
mtext("Percent correct",line=2.5,side=2)
mtext("Sequence length (nucleotides)",side=1,line=2.5)
mtext("Four taxa",cex=1.5,line=1.25)
legend("topleft",pch=c(15,17,16),c("LS","No Dist","MLE"))
box()
dev.off()
